...
export const PWA_PROGRAM_DRAWER_OPEN_KEY = 'pwa-program-drawer-open';
...
const CalendarControlsPwaDrawers = dynamic(
  () =>
    import('@/components/calendar-controls-pwa-drawers').then(
      (m) => m.CalendarControlsPwaDrawers
    ),
  { ssr: false }
);
...
export function CalendarControls({
  ...
  const syncProgramRouteAfterDrawerClose = useCallback(() => {
    const newPath = getRoutePath(selectedProgram as ProgramValue, viewMode);
    if (newPath !== pathname) {
      window.history.replaceState(null, '', newPath);
    }
  }, [selectedProgram, viewMode, pathname]);

  const handleProgramDrawerOpenChange = useCallback(
    (open: boolean) => {
      setIsProgramDrawerOpen(open);
      if (typeof window === 'undefined') return;
      if (open) {
        sessionStorage.setItem(PWA_PROGRAM_DRAWER_OPEN_KEY, '1');
        return;
      }
      sessionStorage.removeItem(PWA_PROGRAM_DRAWER_OPEN_KEY);
      syncProgramRouteAfterDrawerClose();
    },
    [syncProgramRouteAfterDrawerClose]
  );
  ...
  const handleSessionToggle = useCallback((programValue: ProgramValue, sessionId: SessionId, group: 'A' | 'B') => {
    ...
    if (onProgramSessionChange) {
      ...
      if (!drawerOpen && newPath !== pathname) {
        window.history.replaceState(null, '', newPath);
      }
    }
  ...
  const keepDrawerOpenSessionToggle = useCallback(
    (programValue: ProgramValue, sessionId: SessionId, group: 'A' | 'B') => {
      ...
      handleSessionToggle(programValue, sessionId, group);
    },
    [handleSessionToggle]
  );
  ...
  return (
      ...
      >
        ...
        <div className="px-0">
          {isStandalonePwa ? (
            <button
              type="button"
              onClick={() => handleProgramDrawerOpenChange(true)}
      ...
      {isStandalonePwa ? (
        <CalendarControlsPwaDrawers
          isProgramDrawerOpen={isProgramDrawerOpen}
          onProgramDrawerOpenChange={handleProgramDrawerOpenChange}
          isSettingsDrawerOpen={isSettingsDrawerOpen}
          onSettingsDrawerOpenChange={setIsSettingsDrawerOpen}
          ...
          onProgramSelect={keepDrawerOpenProgramSelect}
          onSessionToggle={keepDrawerOpenSessionToggle}
          ...
          showBreak={showBreak}
          showKKT={showKKT}
          hasSemesterPendek={hasSemesterPendek}
          hasKuliahIntersesi={hasKuliahIntersesi}
          hasOthersExams={hasOthersExams}
          hasRegionalDateRange={hasRegionalDateRange}
          isPWAInstalled={isPWAInstalled}
          onFilterToggle={onFilterToggle}
          onShowRegistrationChange={onShowRegistrationChange}
          onShowLectureChange={onShowLectureChange}
          onShowSemesterPendekChange={onShowSemesterPendekChange}
          onShowKuliahIntersesiChange={onShowKuliahIntersesiChange}
          onShowExaminationChange={onShowExaminationChange}
          onShowOthersExamsChange={onShowOthersExamsChange}
          onShowBreakChange={onShowBreakChange}
          onShowKKTChange={onShowKKTChange}
        />
      ) : null}
