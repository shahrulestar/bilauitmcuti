import {
  getRequestCookie,
  jsonResponse,
  withChatVerifiedCookie,
} from "@/lib/chat-api/http";
import {
  askGroq,
...
export async function handleChatPost(request: Request) {
  ...
  const withVerifiedCookie = (response: Response): Response =>
    withChatVerifiedCookie(
      response,
      shouldSetVerifiedCookie,
  ...
  try {
    ...
    const hasVerifiedCookie = getRequestCookie(request, CHAT_TURNSTILE_COOKIE) === "1";
    ...
    if (cachedReply) return withVerifiedCookie(jsonResponse({ reply: cachedReply }));
    ...
    return withVerifiedCookie(jsonResponse({ reply }));
  } catch (error: unknown) {
